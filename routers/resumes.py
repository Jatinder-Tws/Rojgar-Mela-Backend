...
def _map_resume_to_user(user: User, ai_data: dict, parsed_json: dict) -> None:
    ...
    # Keep signup identity fields stable; fill only if missing.
...
def _map_resume_to_portfolio(portfolio: Portfolio, ai_data: dict, parsed_json: dict) -> None:
    for field in (
        "headline", "bio", "date_of_birth", "gender", "city", "state",
        "linkedin_url", "github_url", "website_url", "current_company", "current_role",
    ):
    ...
    if _is_empty(portfolio.work_experiences):
        work = _normalize_list_items(
            ai_data.get("work_experiences") or [],
            {
                "company": None,
                "role": None,
                "start_date": None,
    ...
    if _is_empty(portfolio.education):
        education = _normalize_list_items(
    ...
    if _is_empty(portfolio.certifications):
        certs = _normalize_list_items(
            ai_data.get("certifications") or [],
            {"name": None, "issuer": None, "date": None, "url": None},
        )
        if certs:
            portfolio.certifications = certs

    if _is_empty(portfolio.languages):
        langs = _normalize_list_items(
            ai_data.get("languages") or [],
            {"language": None, "proficiency": "Conversational"},
        )
        if langs:
...
async def _update_resume_processing(resume_id: str, status: str, message: str, **extra) -> None:
    ...
    try:
        async with AsyncSessionLocal() as s:
            ...
            if not res:
                return
            res.parsed_json = _attach_processing_meta(
                res.parsed_json if isinstance(res.parsed_json, dict) else {},
                status,
                message,
                **extra,
            )
...
async def _process_resume(resume_id: str, file_path: str, filename: str) -> None:
    ...
    try:
        ...
        await _update_resume_processing(
            resume_id,
            "embedding",
            "Generating AI embedding for job matching…",
        )
        async with AsyncSessionLocal() as s:
            res = await s.get(Resume, resume_id)
            if not res:
            ...
            if not res.parsed_text:
                ...
                await _update_resume_processing(
                    resume_id,
                    "failed",
                    "Could not extract text from resume. Try a clearer PDF or DOCX.",
                )
                return
        ...
        await _update_resume_processing(
            resume_id,
            "completed",
            "Resume processed. Your profile and job matches are ready.",
        )
    except Exception as e:
        ...
        await _update_resume_processing(
            resume_id,
            "failed",
            "Background job matching failed. Your profile was still saved.",
        )
...
async def upload_resume(
    ...
    processing_seed = _attach_processing_meta(
        parsed_json or {},
        "ai_mapping",
        "AI is mapping your resume data into profile fields…",
        ocr_used=ocr_used,
    )
    ...
    final_parsed = {
        **(parsed_json or {}),
        "ai_profile_data": ai_profile_data,
    }
    final_parsed = _attach_processing_meta(
        final_parsed,
        "queued",
        "Profile saved. Job matching is running in the background…",
        ocr_used=ocr_used,
...
async def get_my_resume_processing_status(
    user: User = Depends(require_seeker),
    db: AsyncSession = Depends(get_db),
):
    ...
    return _resume_status_out(resume)
