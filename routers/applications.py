...
async def apply_to_job(
    ...
    if provider:
        # Find match_id if it exists to allow direct navigation to candidate profile
    ...
    await db.commit()
    # after shortlist commit + seeker notification:
    try:
        await auto_schedule_for_application(db, app, user, job)
        await db.commit()
    except Exception:
        await db.rollback()  # only rollback interview attempt if you use nested logic
        # OR: log and do not fail shortlist — recommended
...
async def get_my_applications(
    ...
    # Join with JobPosting to get all metadata for the cards
...
async def get_job_applications(
    ...
    for app, seeker in rows:
        ...
        # Populate seeker details if available
        if seeker:
...
async def get_all_external_applications(
    user: User = Depends(require_provider),
    db: AsyncSession = Depends(get_db),
):
    """
    Consolidated list of all external/guest applicants for a provider.
    Includes both job-specific applicants and global leads (no job_id).
    """
    result = await db.execute(
    ...
    for candidate, job in rows:
        # Create ApplicationOut compatible object
...
async def get_shortlisted_applications(
    ...
    # Fetch all shortlisted applications for jobs owned by this provider
    result = await db.execute(
...
async def reject_application(
    ...
    # Fetch seeker's resume for AI feedback
    seeker_result = await db.execute(select(User).where(User.id == app.seeker_id))
    seeker = seeker_result.scalar_one_or_none()

    resume_result = await db.execute(
        select(Resume)
        .where(Resume.user_id == app.seeker_id)
    ...
    async def _generate_and_save_feedback():
        ...
        async with AsyncSessionLocal() as s:
            ...
            # Notify seeker within the active session
            if seeker:
                await create_notification(
                    db=s,
                    user_id=seeker.id,
...
async def update_application_status(
    ...
    try:
        ...
        # Fetch application first to avoid transaction issues
        ...
        if status == "shortlisted":
            ...
            # Default case: update status and optional ai_feedback
            app.status = status
            if ai_feedback:
        ...
        # Re-raise HTTP exceptions without rolling back (they're not database errors)
        await db.rollback()
        raise
