...
# Cost per token for Gemini API (USD per token)
COST_PER_TOKEN = 0.00035 / 1000  # Adjust based on actual pricing
...
from services.auth_service import get_current_user, create_access_token, hash_password
...
async def get_next_question(session_id: str, db: AsyncSession = Depends(get_db)):
    ...
    # Determine if assessment is complete (max 15 questions)
    is_complete = len(session.qa_history or []) >= 15
    if is_complete:
        # No more questions; indicate completion
        return {"is_complete": True}

    # Generate next question
    question_json, tokens = await generate_next_assessment_question(
        session.qa_history,
        session.experience_level,
        session.domain_interest,
    )
    # If the AI returns a list, take the first question
...
async def submit_answer(session_id: str, body: AssessmentAnswer, db: AsyncSession = Depends(get_db)):
    ...
    # Update history - SQLAlchemy doesn't track mutation of JSON in-place well for async sometimes
    # So we assign a new list object
    ...
    # Ensure we have a dict for a single question
    if isinstance(question_json, list):
...
async def complete_assessment(session_id: str, body: AssessmentComplete, db: AsyncSession = Depends(get_db)):
    ...
    if not user:
        ...
        # Existing user (maybe trying to redo assessment)
    ...
    # Generate evaluation
    eval_data = await generate_assessment_evaluation(
        session.qa_history,
        session.experience_level,
        session.domain_interest
    )
    # Track final evaluation token usage and update session
    ...
    token = create_access_token(data={"sub": user.id}) # Using ID for sub is better
    # 4. Return QR code for setup
...
async def link_to_profile(session_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not assessment_result:
        ...
        # If no result exists yet, generate it now
        eval_data = await generate_assessment_evaluation(
            session.qa_history,
            session.experience_level,
            session.domain_interest
    ...
    cost=session.cost
        ...
        # Just update the user_id if it's currently null
