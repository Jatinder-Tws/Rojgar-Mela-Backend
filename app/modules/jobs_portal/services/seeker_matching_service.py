...
async def _publish_sse_event(user_id: str, data: dict) -> None:
    try:
        import json
...
async def proactive_match_resume_to_jobs(resume_id: str) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Broadcast update to seeker's dashboard via SSE
...
async def proactive_match_job_to_candidates(job_id: str) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Broadcast update to provider's dashboard via SSE
            await _publish_sse_event(
                str(job.provider_id),
...
async def _cleanup_job_matches_and_notify(job_id: str, job_title: str) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Broadcast dashboard update to all affected seekers via SSE
            for sid in seeker_ids:
                await _publish_sse_event(
                    sid,
