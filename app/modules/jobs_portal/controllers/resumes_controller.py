...
from app.modules.jobs_portal.schemas.resume import ResumeResponse
...
from app.modules.jobs_portal.services.ai_improvement_suggestion_service import analyze_resume_multi
...
async def _process_resume(resume_id: str, file_path: str, filename: str) -> None:
    try:
        from app.core.database import AsyncSessionLocal
        from app.modules.jobs_portal.services.seeker_matching_service import embed_and_store_resume, proactive_match_resume_to_jobs
...
async def upload_resume(background_tasks: BackgroundTasks, file: UploadFile, user: User, db: AsyncSession) -> ResumeOut:
    ...
    try:
        # parse_resume can fall back to synchronous pytesseract OCR (up to 10 pages),
        # which would otherwise block this single-worker event loop for every other
        # in-flight request. Offload to a worker thread, same pattern as
        # hash_password/verify_password in app/core/security.py.
        raw_text, parsed_json = await asyncio.to_thread(parse_resume, file_path, filename)
    except Exception as e:
...
async def improve_resume(request: ResumeImproveRequest, db: AsyncSession) -> dict:
    ...
    from app.shared.services.celery_tasks import improve_resume_task
