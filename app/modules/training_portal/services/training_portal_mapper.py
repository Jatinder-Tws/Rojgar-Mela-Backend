...
def enrollment_to_out(row: TrainingPortalEnrollment) -> PortalEnrollmentOut:
    return PortalEnrollmentOut(
        ...
        discount_amount=getattr(row, "discount_amount", 0.0) or 0.0,
        ...
        token_amount=getattr(row, "token_amount", 0.0) or 0.0,
        token_paid_at=getattr(row, "token_paid_at", None),
        venue_visit_deadline=getattr(row, "venue_visit_deadline", None),
        token_expired=bool(getattr(row, "token_expired", False)),
        refunded_at=getattr(row, "refunded_at", None),
        refund_reason=getattr(row, "refund_reason", None),
