...
class TrainingPortalEnrollment(Base):
    ...
    token_amount = Column(Float, nullable=False, default=2000.0)
    token_paid_at = Column(DateTime, nullable=True)
    venue_visit_deadline = Column(DateTime, nullable=True)
    token_expired = Column(Boolean, nullable=False, default=False, index=True)
    refunded_at = Column(DateTime, nullable=True)
    refund_reason = Column(Text, nullable=True)
    refunded_by_id = Column(UUID(as_uuid=False), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
