...
from app.modules.super_admin.services.support_bot_knowledge import build_system_prompt
...
# In-memory sessions with TTL pruning
_sessions: dict[str, BotSession] = {}


def _prune_expired_sessions(max_age_seconds: int = 7200) -> None:
    now = datetime.utcnow()
    expired = [
...
def get_or_create_session(user_id: str, user_role: str) -> BotSession:
    ...
    _sessions[user_id].last_activity = datetime.utcnow()
...
async def _create_ticket_from_session(
    ...
    from app.modules.super_admin.controllers.support_controller import _message_to_out
